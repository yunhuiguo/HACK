...
def h_tanh(x, clip_norm):   

    return x.clamp(-clip_norm, clip_norm).tanh()
...
class RiemannianGradient(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        RiemannianGradient.c = RiemannianGradient.c
...
class Arsinh(torch.autograd.Function):
    ...
    def forward(ctx, x):
        ...
        return torch.log( (x + torch.sqrt_(1 + x.pow(2))).clamp_min_(1e-5))
...
def expmap0(u, *, c=1.0, clip_norm=None):
    r"""
    ...
    return _expmap0(u, c, clip_norm)


def _expmap0(u, c, clip_norm):

    sqrt_c = c ** 0.5

    u_norm = torch.clamp_min( u.norm(dim=-1, p=2, keepdim=True), 1e-5)

    gamma_1 = h_tanh(sqrt_c * u_norm, clip_norm) * u / (sqrt_c * u_norm)
...
def _hyperbolic_softmax(X, A, P, c):
    lambda_pkc = 2 / (1 - c * P.pow(2).sum(dim=1) + eps_norm)
    
    k = lambda_pkc * (torch.norm(A, dim=1) + eps_norm) / torch.sqrt(c)
    
    mob_add = _mobius_addition_batch(-P, X, c)
    
    num = 2 * torch.sqrt(c) * torch.sum(mob_add * (A.unsqueeze(1) + eps_norm), dim=-1)
    
    denom = (torch.norm(A, dim=1, keepdim=True) + eps_norm) * (1 - c * mob_add.pow(2).sum(dim=2))
...
def dist_to_hyperplane(X, A, P, c):

    k = 1.0 / torch.sqrt(c)

    mob_add = _mobius_addition_batch(-P, X, c)

    num = 2 * torch.sqrt(c) * torch.abs(torch.sum(mob_add * A.unsqueeze(1), dim=-1))
    denom = torch.norm(A, dim=1, keepdim=True) * (1 - c * mob_add.pow(2).sum(dim=2))
 
    dist = k * arsinh(num / denom)
...
def Eculidean_to_Hyperbolic(Euclidean_r, curvature=1.0):
    """
    ...
    return s * math.log((s+Euclidean_r)/(s-Euclidean_r))


def Hyperbolic_to_Eculidean(r, curvature=1.0):
    ...
    return (math.exp(r)-s)/ (math.exp(r)+s)


def h_area(r, curvature):
    """
    Compute the area of hyperbolic ball centered at origin with hyperbolic radius r
    """

    s = 1 / (math.sqrt(curvature))

    return 4 * math.pi * s * s * sinh(r/(2*s)) * sinh(r/(2*s)) 


def area_to_r(area, curvature):

    """
    Given the hyperbolic area, compute the hyperbolic radius
    """

    s = 1 / (math.sqrt(curvature))

    tmp = math.sqrt(area / (4 * math.pi * s * s))

    tmp = inverse_sinh(tmp)
...
def pairwise_poincare_distances(X, Y):
    n = X.size()[0]

    p_inner_prod = Pairwise_inner_product(X, Y)
    ##################

    norm_2_X = torch.sum(X * X, dim=-1)

    norm_2_X = norm_2_X.unsqueeze(1).repeat(1, n)

    norm_2_Y = torch.sum(Y * Y, dim=-1)

    norm_2_Y = norm_2_Y.unsqueeze(0).repeat(n, 1)
    ##################

    norm_XY =  norm_2_X * norm_2_Y

    p_wise_dist  = 1 + 2*(norm_2_X + norm_2_Y - 2 * p_inner_prod) / (1 - norm_2_X - norm_2_Y + norm_XY)

    p_wise_dist = torch.max(torch.tensor(1.00001), p_wise_dist) 

    poincare_dist_m = torch.arccosh(p_wise_dist)

    return poincare_dist_m
